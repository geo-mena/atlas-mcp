...
async function exerciseMcpServer(
  ...
  if (!initResp || initResp.error) {
    throw new Error(`${name}: initialize failed: ${JSON.stringify(initResp?.error ?? 'no response')}`);
  }
  if (!toolsResp || toolsResp.error) {
    throw new Error(`${name}: tools/list failed: ${JSON.stringify(toolsResp?.error ?? 'no response')}`);
  }
...
async function exerciseHookScenarios(): Promise<void> {
  ...
  const scenarios = [
    {
      name: 'intra-run',
      stdin: { cwd: '/tmp', tool_name: 'Write', tool_input: { file_path: '/tmp/.atlas/runs/r1/scratch.json' } },
      expectedExit: 0,
      reportContent: null as string | null,
    },
    {
      name: 'no-run',
      stdin: { cwd: TEST_DIR, tool_name: 'Write', tool_input: { file_path: `${TEST_DIR}/foo.txt` } },
      expectedExit: 0,
      reportContent: null,
    },
    {
      name: 'PASS-verdict',
      stdin: { cwd: TEST_DIR, tool_name: 'Write', tool_input: { file_path: `${TEST_DIR}/promoted.txt` } },
      expectedExit: 0,
  ...
  for (const scenario of scenarios) {
    ...
    if (exit !== scenario.expectedExit) {
      throw new Error(`hook scenario ${scenario.name}: expected exit ${scenario.expectedExit}, got ${exit}`);
    }
  }
}
