...
function main(): void {
  ...
  try {
    ...
    const parsedSpec = parseYaml(openapi.yaml) as { openapi: string; paths: Record<string, unknown> };
    assert.equal(parsedSpec.openapi, '3.1.0', 'openapi 3.1 expected');
    assert.ok(openapi.path_count > 0, 'expected ≥ 1 OpenAPI path');

    const mcpServer = emitMcpServer(merged, { runId: RUN_ID, outDir: join(artifactsDir, 'mcp-server') });
    ...
    const testSuite = emitTestSuite(merged, { runId: RUN_ID, outDir: join(artifactsDir, 'tests') });
    ...
    assert.equal(auditResult.run_verdict, 'PASS', `expected audit PASS, got ${auditResult.run_verdict}`);
...
function seedSourceFacts(scratchpad: Scratchpad): number {
  const facts = [
    ...
    },
    {
      ...
      fact_type: 'ui_field',
      content: { screen: '/ve/invoice', name: 'customer_id', label: 'Customer', input_type: 'select', required: true },
    ...
    },
    {
      ...
      content: { scenario_id: 'happy', method: 'POST', url: 'http://localhost:8080/ve/invoice' },
      evidence_uri: 'har://golden#1',
    },
    {
      source_agent: 'traffic-sniffer' as const,
      fact_type: 'http_response',
      content: { scenario_id: 'happy', status: 302, method: 'POST', url: 'http://localhost:8080/ve/invoice' },
      evidence_uri: 'har://golden#1',
    },
    {
      source_agent: 'doc-harvester' as const,
