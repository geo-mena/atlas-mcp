...
const TOOL_DEFINITIONS = [
  ...
  {
    name: 'merged_facts',
    description: 'Read merged_facts for a run, optionally filtered by fact_type and resolution.',
    inputSchema: {
      type: 'object',
      required: ['run_id'],
      properties: {
  ...
  {
    name: 'conflicts',
    description: 'Shortcut for merged_facts({ run_id, resolution: "unresolved" }) — surfaces only the items needing human review.',
    inputSchema: {
      type: 'object',
      required: ['run_id'],
      properties: { run_id: { type: 'string', minLength: 1 } },
