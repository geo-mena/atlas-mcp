...
export function listMergedFacts(config: ToolsConfig, raw: unknown): { merged_facts: ReturnType<Scratchpad['selectMergedFacts']> } {
...
export function listConflicts(config: ToolsConfig, raw: unknown): {
  conflicts: ReturnType<Scratchpad['selectMergedFacts']>;
} {
  const input = parseOrThrow(ConflictsInputSchema, raw);
  const scratchpad = openScratchpad(config);
  try {
    const conflicts = scratchpad.selectMergedFacts({ run_id: input.run_id, resolution: 'unresolved' });
    return { conflicts };
  } finally {
    scratchpad.close();
  }
...
function openScratchpad(config: ToolsConfig): Scratchpad {
  try {
    ...
    throw new SynthesizerError('SCRATCHPAD_UNREACHABLE', `cannot open scratchpad at ${config.scratchpadPath}: ${message}`);
  }
}
