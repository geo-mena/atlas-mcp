...
export function parseHar(text: string): HarFile {
  ...
  if (!isHarFile(parsed)) {
    throw new TrafficSnifferError('HAR_PARSE_ERROR', 'JSON does not match HAR shape (missing log.entries)');
  }
  return parsed;
}
...
export function summarizeHar(entries: readonly HarEntry[]): { count: number; methods: Record<string, number> } {
