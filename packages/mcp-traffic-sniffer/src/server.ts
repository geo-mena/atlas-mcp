...
const TOOL_DEFINITIONS = [
    ...
    },
    {
        name: 'stop_proxy',
        description:
            'Terminate the mitmdump subprocess for the given run_id. The HAR file remains on disk for downstream analysis.',
        inputSchema: {
            type: 'object',
            required: ['run_id'],
    ...
    },
    {
        name: 'dump_har',
        description:
            'Read the HAR file for the given run_id and return a summary (entry count, distinct endpoints).',
        inputSchema: {
            type: 'object',
            required: ['run_id'],
            properties: { run_id: { type: 'string', minLength: 1 } },
