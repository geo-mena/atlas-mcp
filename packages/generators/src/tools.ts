...
export function runEmitOpenApi(config: ToolsConfig, raw: unknown): OpenApiResult & { written_to: string } {
  ...
  const outPath = join(input.out_dir ?? defaultArtifactsDir(config, input.run_id), 'openapi.yaml');
...
function readMergedFacts(config: ToolsConfig, runId: string): ReturnType<Scratchpad['selectMergedFacts']> {
  ...
  try {
    ...
    if (facts.length === 0) {
      throw new GeneratorError('NO_MERGED_FACTS', `no merged_facts for run_id ${runId}; run synthesize first`);
    }
    return facts;
  } finally {
    scratchpad.close();
