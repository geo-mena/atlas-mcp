...
const TOOL_DEFINITIONS = [
  {
    name: 'emit_openapi',
    description: 'Generate an OpenAPI 3.1 spec at <out_dir>/openapi.yaml from merged_facts. Defaults out_dir to .atlas/runs/<run_id>/artifacts/.',
    inputSchema: {
      type: 'object',
      required: ['run_id'],
      properties: {
  ...
  },
  {
    ...
    description: 'Generate a TypeScript MCP server scaffold at <out_dir>/mcp-server/ from merged_facts. One tool per route fact.',
    inputSchema: {
      type: 'object',
      required: ['run_id'],
      properties: {
  ...
  },
  {
    ...
    description: 'Generate a Vitest + nock test suite at <out_dir>/tests/ from merged_facts. One case per http_request/http_response scenario.',
    inputSchema: {
      type: 'object',
      required: ['run_id'],
      properties: {
