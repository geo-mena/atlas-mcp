...
function buildPaths(
    ...
    const controllersByName = indexBy(
        controllers,
        (f) => `${str(f.content, 'class')}@${str(f.content, 'method')}`,
    );
    const fieldsByEndpoint = groupBy(payloadFields, (f) => str(f.content, 'endpoint'));
    const responsesByEndpoint = groupBy(
        httpResponses,
        (f) => `${str(f.content, 'method').toUpperCase()} ${str(f.content, 'url').toLowerCase()}`,
    );
    ...
    for (const route of routes) {
        ...
        const requestBody = buildRequestBody(
            fieldsByEndpoint.get(`${method.toUpperCase()} ${path}`),
        );
        if (requestBody !== null) {
        ...
        const responses = buildResponses(
            responsesByEndpoint.get(`${method.toUpperCase()} ${path}`),
        );
...
function buildRequestBody(
    fields: readonly MergedFact[] | undefined,
...
function buildResponses(
    responses: readonly MergedFact[] | undefined,
