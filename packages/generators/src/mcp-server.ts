 ...
 */
export function emitMcpServer(facts: readonly MergedFact[], options: McpServerOptions): McpServerResult {
  ...
  written.push(write(options.outDir, 'package.json', renderPackageJson(options.serverName ?? `atlas-generated-${options.runId}`)));
...
function buildTool(route: MergedFact, payloadFields: readonly MergedFact[]): ToolSpec {
  ...
  const fieldsForEndpoint = payloadFields.filter(
    (f) => `${String(f.content['method'] ?? '').toUpperCase()} ${String(f.content['endpoint'] ?? '')}` === endpoint
      || String(f.content['endpoint'] ?? '') === endpoint,
  );
...
function toolName(method: string, path: string): string {
  const slug = path.replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').toLowerCase();
...
function renderReadme(options: McpServerOptions, toolCount: number): string {
  return `# ${options.serverName ?? `atlas-generated-${options.runId}`}\n\n` +
