...
function merged(input: Partial<MergedFactInput>): MergedFact {
    const base: MergedFactInput = {
        ...
        winning_source:
            input.winning_source === undefined ? 'traffic-sniffer' : input.winning_source,
        ...
        const result = emitTestSuite(
            [
                merged({
                    ...
                    content: { scenario_id: 's1', method: 'POST', url: 'http://x/y' },
                }),
    ...
    it('emits one vitest case per paired scenario', () => {
        emitTestSuite(
            [
                merged({
                    fact_type: 'http_request',
                    content: { scenario_id: 'a', method: 'GET', url: 'http://x/a' },
                }),
                merged({ fact_type: 'http_response', content: { scenario_id: 'a', status: 200 } }),
                merged({
                    fact_type: 'http_request',
                    content: { scenario_id: 'b', method: 'POST', url: 'http://x/b' },
                }),
    ...
    it('drops orphaned requests with no matching response', () => {
        const result = emitTestSuite(
            [
                merged({
                    fact_type: 'http_request',
                    content: { scenario_id: 'orphan', method: 'GET', url: 'http://x' },
                }),
            ],
            { runId: 'r', outDir: tmp },
        );
