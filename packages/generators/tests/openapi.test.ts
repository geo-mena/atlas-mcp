...
describe('emitOpenApi', () => {
    ...
    it('includes x-atlas-evidence with source fact ids and winning source', () => {
        ...
        const parsed = parseYaml(result.yaml) as {
            paths: Record<
                ...
                Record<
                    string,
                    { 'x-atlas-evidence': { source_fact_ids: number[]; winning_source: string } }
                >
            >;
        };
    ...
    it('builds component schemas from field_definition facts with type coercion', () => {
        const facts = [
            ...
            merged({
                fact_type: 'field_definition',
                content: { name: 'currency', type: 'string' },
            }),
            merged({
                fact_type: 'field_definition',
                content: { name: 'quantity', type: 'integer' },
            }),
        ];
        const result = emitOpenApi(facts, { runId: 'r' });
        const parsed = parseYaml(result.yaml) as {
    ...
    it('seeds responses from observed http_response status codes', () => {
        ...
        expect(Object.keys(parsed.paths['/ve/invoice']?.['post']?.responses ?? {}).sort()).toEqual([
            '302',
            '422',
        ]);
    });
