...
import { auditScenarios, type AuditOptions, type AuditResult, type ScenarioInput } from './audit.js';
...
function buildAuditOptions(input: z.infer<typeof AuditInputSchema>): AuditOptions {
  const normalization = input.normalization
    ? {
        ...(input.normalization.scrub_paths !== undefined ? { scrub_paths: input.normalization.scrub_paths } : {}),
  ...
  const classify =
    input.noise_allowlist !== undefined || input.text_noise_max !== undefined
      ? {
          ...(input.noise_allowlist !== undefined ? { noise_allowlist: input.noise_allowlist } : {}),
          ...(input.text_noise_max !== undefined ? { text_noise_max: input.text_noise_max } : {}),
        }
      : undefined;
...
type NormalizedCt = NonNullable<Parameters<typeof auditScenarios>[1]['normalization']> extends { masks?: infer M }
