...
export function writeReports(result: AuditResult, auditDir: string): ReportPaths {
    try {
        ...
        throw new AuditorError(
            'WRITE_FAILED',
            `cannot create audit directory at ${auditDir}: ${message}`,
        );
    }
...
function renderReportMd(result: AuditResult): string {
    ...
    if (review.length > 0) {
        ...
        for (const r of review) {
            lines.push(
                `- \`${r.scenario_id}\` — ${r.request.method} ${r.request.url}: ${r.reason}`,
            );
        }
        lines.push('');
    }
...
function renderCoverageMd(result: AuditResult): string {
    ...
    for (const scenario of result.scenarios) {
        ...
        const bucket = byContentType.get(ct) ?? {
            PASS: 0,
            'PASS-WITH-NOISE': 0,
            'HUMAN-REVIEW': 0,
            FAIL: 0,
    ...
    for (const [ct, counts] of [...byContentType.entries()].sort()) {
        lines.push(
            `| \`${ct}\` | ${counts.PASS} | ${counts['PASS-WITH-NOISE']} | ${counts['HUMAN-REVIEW']} | ${counts.FAIL} |`,
        );
    }
    lines.push('');
    return lines.join('\n');
