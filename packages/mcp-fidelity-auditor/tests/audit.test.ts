...
function jsonResponse(status: number, payload: unknown): { status: number; content_type: string; body: string } {
...
describe('auditScenarios', () => {
  ...
  it('PASS-WITH-NOISE when changes are inside the noise allowlist', () => {
    const result = auditScenarios(
      [scenario('a', { x: 1, ts: 'old' }, { x: 1, ts: 'new' })],
      { run_id: 'r', classify: { noise_allowlist: ['$.ts'] } },
    );
  ...
  it('HUMAN-REVIEW when partial allowlist coverage', () => {
    const result = auditScenarios(
      [scenario('a', { x: 1, ts: 'old' }, { x: 2, ts: 'new' })],
      { run_id: 'r', classify: { noise_allowlist: ['$.ts'] } },
    );
...
describe('writeReports', () => {
  ...
  it('writes results.jsonl, report.md, coverage.md, and failed/ payloads', () => {
    const result = auditScenarios(
      [
        scenario('pass', { x: 1 }, { x: 1 }),
        scenario('fail', { x: 1 }, { x: 2 }),
      ],
      { run_id: 'r' },
    );
