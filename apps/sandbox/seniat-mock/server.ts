...
app.post('/seniat-mock/authorize', (req: Request, res: Response) => {
  ...
  if (FLAKE_EVERY_N > 0 && requestCounter % FLAKE_EVERY_N === 0) {
    res.status(503).set('Content-Type', 'application/xml').send(buildErrorEnvelope('SERVICE_UNAVAILABLE', 'Synthetic flake'));
    return;
  }

  const body = typeof req.body === 'string' ? req.body : '';
  if (!body.includes('<') || body.length < 16) {
    res.status(400).set('Content-Type', 'application/xml').send(buildErrorEnvelope('MALFORMED_REQUEST', 'Envelope is empty or not XML'));
    return;
  }

  controlCounter += 1;
  res.status(200).set('Content-Type', 'application/xml').send(buildSuccessEnvelope(controlCounter, requestCounter));
});
...
function buildSuccessEnvelope(controlNumber: number, fiscalSequence: number): string {
  return create({ version: '1.0' })
    ...
    .ele('authorization')
      .ele('status').txt('AUTHORIZED').up()
      .ele('control_number').txt(`VE-${controlNumber}`).up()
      .ele('fiscal_sequence').txt(String(fiscalSequence).padStart(10, '0')).up()
      .ele('issued_at').txt(new Date().toISOString()).up()
      .up()
    .ele('signature', { xmlns: 'http://www.w3.org/2000/09/xmldsig#' })
      .ele('SignatureValue').txt('SYNTHETIC-SIGNATURE-DAY0').up()
      .ele('KeyInfo').ele('KeyName').txt('atlas-sandbox-key').up().up()
      .up()
